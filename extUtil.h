...
typedef struct {
  ...
  char  free;   /* 1: free data of current dataset if a new one is selected   */
...
typedef struct {
  ...
  int pFlag;             /* textpos 0: last, 1: bottom, 2: front 3: top */
...
typedef struct {
  char name[MAX_LINE_LENGTH];          // setname with that boundary condition (in,out etc)
  char bctype[MAX_LINE_LENGTH];        // subsonic-inflow etc, as given in send command
  int surfs;                           // nr of surfaces (lines) in that set
  int *surf;                           // duns surface-nr (line)
  int *nBlock;                         // duns block-nr (cgx surf or body index +1)
  int *side;                           // cgx surf or body edge index
} NodeBlockbou;
...
typedef struct {
  ...
  fpos_t *filepntr;    /* points to the actual file pos.  (fgetpos) */
  long byte_offset;    /* position after the stop command, unused */   
}CommandFile;
...
typedef struct {
  ...
  int   numnod;              /* number of nodes in datablock */
  char  **compName;
  char  **icname;
  char  name[MAX_LINE_LENGTH];
...
typedef struct {
  char  *name;
  int type;              /* 0:plane, 1:cyl, 2: cone, 3:sph, 4:nurbs 5:torus */
  int p[7];              /* 0:3p, 1:3p, 2:4p 3:7p 4:p[0]=nurbsindx 5:4p*/
...
typedef struct {
  char  *name;
  char  *string;
  char  flag;     // if 1 not to be written to fbd file
} Values;
...
typedef struct {
  ...
  int   sref;             /* if nl!=4: index of the 4 sided substitute surface, else -1 */
  double cx;               /* CG, in orient determined, not scaled during run! */
  double cy;
  double cz;
  int   fail;             /* if 1 then the meshing failed, if 2 trimming had failed */
  int   permElem;         /* if 1 the mesher will generate elements, else only nodes */
  int   nn;
  int   *nod;
  int   ne;
  int   *elem;
...
typedef struct {
  char  *name;
  char  typ;     /* 0: regular, 1:with unbalanced edges */
...
int readIsaac(char *datin, Summen *anz, Nodes **nptr, Elements **eptr, Datasets **lptr, NodeBlocks **bptr, int elem_type);
...
void v_print( char *text, double *a);
...
int write2aba(char *datout, Summen *anz, Nodes *node, Elements *elem, Datasets *lcase, char **dat, int controlFlag );
...
int write2isaac( char *datout, Summen *anz, Nodes *node, Elements *elem, Datasets *lcase, NodeBlocks *nBlock, int bouNr, NodeBlockbou *blockbou );
int write2duns( char *datout, Summen *anz, Nodes *node, Elements *elem, Datasets *lcase, NodeBlocks *nBlock, int bouNr, NodeBlockbou *blockbou );
